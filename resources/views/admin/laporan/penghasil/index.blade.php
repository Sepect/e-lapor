...
@section('title', 'Laporan Penghasil')
@section('subtitle', 'Laporan Penghasil')
...
@section('content')
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
        <div class="card-body p-4 d-flex flex-column flex-md-row align-items-start align-items-md-center gap-3 bg-white position-relative">
            ...
            <div class="flex-grow-1 z-1">
                <h5 class="fw-bold mb-1 text-dark">Laporan Penghasil Limbah B3</h5>
                <p class="text-muted mb-0 small">Cetak laporan berdasarkan data penghasil limbah B3 terdaftar.</p>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white border-bottom p-4 d-flex align-items-center">
            ...
            <div>
                <h6 class="mb-0 fw-bold text-dark">Cetak Laporan Penghasil</h6>
        ...
        <div class="card-body p-4 p-md-5">
            <form action="{{ route('admin.laporan.cetak') }}" method="POST">
                @csrf
                <input type="hidden" name="jenis_laporan" value="penghasil">
                ...
                <div class="row mb-4">
                    <label class="col-md-3 form-label-custom">NAMA PENGHASIL</label>
                    <div class="col-md-6 col-12">
                        <div class="input-group">
                            <span class="input-group-text bg-light border-end-0"><i class="fas fa-building text-muted"></i></span>
                            <select class="form-select border-start-0 bg-light" name="penghasil">
                                <option value="" selected disabled>Pilih Instansi / Perusahaan</option>
                                @foreach($daftarPenghasil as $p)
                                    <option value="{{ $p->id_user }}" {{ old('penghasil') == $p->id_user ? 'selected' : '' }}>

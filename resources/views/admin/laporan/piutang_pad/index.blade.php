...
@section('title', 'Laporan Piutang PAD')
@section('subtitle', 'Laporan Piutang PAD')
...
@section('content')
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
        <div class="card-body p-4 d-flex flex-column flex-md-row align-items-start align-items-md-center gap-3 bg-white position-relative">
            <div class="position-absolute top-0 start-0 w-100 h-100 bg-danger opacity-10" style="pointer-events: none;"></div>
            <div class="position-absolute top-0 start-0 h-100 bg-danger" style="width: 6px;"></div>

            <div class="bg-danger bg-gradient text-white rounded-circle d-flex justify-content-center align-items-center flex-shrink-0 shadow-sm z-1"
                 style="width: 55px; height: 55px;">
                <i class="fas fa-exclamation-circle fs-4"></i>
            </div>
            <div class="flex-grow-1 z-1">
                <h5 class="fw-bold mb-1 text-dark">Laporan Piutang PAD</h5>
                <p class="text-muted mb-0 small">Cetak laporan piutang Pendapatan Asli Daerah dari tagihan yang belum dibayar.</p>
            </div>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white border-bottom p-4 d-flex align-items-center">
            ...
            <div>
                <h6 class="mb-0 fw-bold text-dark">Cetak Piutang PAD</h6>
        ...
        <div class="card-body p-4 p-md-5">
            <form action="{{ route('admin.laporan.cetak') }}" method="POST">
                @csrf
                <input type="hidden" name="jenis_laporan" value="piutang_pad">
